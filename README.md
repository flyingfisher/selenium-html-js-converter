...
Pass the dimensions as the first argument, as width times height. Width and height can be separated by anything non-numerical, so `setWindowSize('450 800')`, `setWindowSize('450, 800')`, `setWindowSize('450x800')`, and `setWindowSize('450 pixels on the wide side and 800 pixels on the high one, please')` are all valid arguments.